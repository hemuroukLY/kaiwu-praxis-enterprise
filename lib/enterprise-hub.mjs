...
async function handle(req, res) {
  ...
  if (req.method === 'POST' && url.pathname === '/api/batch') {
    ...
    if (!terminalIds.length || !workerIds.length || !allowedActions.has(payload.action) || !String(payload.name || '').trim()) return send(res, 400, { ok: false, error: 'invalid batch request' })
