...
function visibleState() {
  ...
  return { version: VERSION, enterpriseName: ENTERPRISE_NAME, hubHost: HUB_HOST, hubPort: HUB_PORT, terminals, audit, commandSummary }
}
...
async function handle(req, res) {
  ...
  if (req.method === 'POST' && url.pathname === '/api/terminal/heartbeat') {
    ...
    const now = new Date().toISOString()
    database.prepare('UPDATE terminals SET name=?,host_name=?,port=?,version=?,workers_json=?,session_count=?,sessions_json=?,remote_address=?,updated_at=?,last_seen=? WHERE id=?')
      .run(String(data.name || terminal.name).slice(0, 160), String(data.hostname || '').slice(0, 160), Number(data.port) || 0, String(data.version || ''), JSON.stringify(data.workers || {}), Number(data.sessionCount) || 0, JSON.stringify(data.sessionsByWorker || {}), String(req.socket.remoteAddress || ''), now, now, data.id)
  ...
  if (req.method === 'POST' && url.pathname === '/api/batch') {
    ...
    const allowedActions = new Set(['addKnowledge', 'removeKnowledge', 'addSop', 'removeSop', 'addSkill', 'removeSkill', 'enableTool', 'disableTool'])
    if (!terminalIds.length || !workerIds.length || !allowedActions.has(data.payload?.action)) return send(res, 400, { ok: false, error: 'invalid batch request' })
    ...
    try {
      ...
      for (const terminalId of terminalIds) insert.run(`command-${randomUUID()}`, batchId, terminalId, JSON.stringify(data.payload), JSON.stringify(workerIds), 'queued', now, expiresAt)
      appendAudit({ id: batchId, action: data.actionLabel || data.payload.action, target: `${terminalIds.length} 个终端 / ${workerIds.length} 类员工`, detail: String(data.detail || '').slice(0, 1000), result: '待执行' })
      database.exec('COMMIT')
    } catch (error) {
      database.exec('ROLLBACK')
      throw error
...
export async function ensureEnterpriseHub(logger) {
  ...
  hubPromise = (async () => {
    ...
    logger?.info?.(`kaiwu-praxis-enterprise: hub ready at ${HUB_URL}; data=${DATA_FILE}`)
    return { url: HUB_URL, server, database, dataFile: DATA_FILE }
  })()
  return hubPromise
}

export { DATA_FILE, HUB_URL }
