...
```bash
KAIWU_ENTERPRISE_HUB_HOST=0.0.0.0
...
企业管理页面首次进入「接入管理」，填写中枢地址和管理员令牌。未鉴权的浏览器会自动进入接入管理，不再显示容易误解的空总览；顶栏“使用引导”包含完整接入步骤和总览显示 0 时的排查说明。管理员接口必须使用 Bearer 令牌；员工端注册后取得独立终端令牌。注册码只保存摘要并受有效期、最大使用次数限制。
