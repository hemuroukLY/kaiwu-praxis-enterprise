...
const remoteBase = `http://127.0.0.2:${port}`
const adminToken = 'test-admin-token'

function startHub() {
  const child = spawn(process.execPath, ['scripts/hub-runner.mjs'], {
    cwd: new URL('..', import.meta.url),
    env: { ...process.env, KAIWU_ENTERPRISE_HUB_HOST: '127.0.0.2', KAIWU_ENTERPRISE_HUB_PORT: String(port), KAIWU_ENTERPRISE_DATA_DIR: dataDir, KAIWU_ENTERPRISE_ADMIN_TOKEN: adminToken },
...
try {
  ...
  assert.equal((await fetch(remoteBase + '/api/health')).status, 200)
  ...
  assert.equal(command.payload.name, '跨设备验收资料')
  ...
  console.log(JSON.stringify({ ok: true, checks: 20, dataDir, terminalId: state.terminals[0].id, commandSummary: state.commandSummary }, null, 2))
} finally {
